var load_quiz = function(level, qid)
{
    var trial = 0;
    var input = 0;
    var sentence = qlist[level][qid];
    if (!sentence) {
        menu();
        return;
    }
    ...
    // 初期画面
    $("#qid").html(qid + 1).unbind().dblclick(function(){ voice_finished(); });
    $("#level").text("★".repeat(level + 1));
    $("#hint").text("");
    var hint = sentence.match(/\[.+?\]/g);
    if (hint) {
        $("#hint").text("Keyword: " + hint.join(";") + " ");
        sentence = sentence.split("[").join("").split("]").join("");
    }
    if (sentence.match(/[0-9]/)) $("#hint").append("(Use the Arabic numbers)");
    $("#speed").text(100).show();
    $("#result, #ending, #next, #stspeed").hide();
    $("#answer").val("").prop("disabled", true);
    $(".co").text(sentence);// .show();
    $("#point, .mark, .ans").html("");
    sentence.split(" ").forEach(function(c) {
        $("#point").append("<span class=rate>0</span> ");
    });
    $("#point").append("<div id=todo>Retry!</div>");

    //再生終了時イベント
    var voice_finished = function() {
        if (!$("#answer").prop("disabled")) return;
        input = 0;
        $("#input").hide();
        trial++;
        $("#listen, #answer").prop("disabled", false);
        $("#answer").focus();
        $("#giveup").show();
        if (trial < 5) return;
        $("#listen").unbind();
    ...
    $("#listen").focus().unbind().click(function() {
        ...
        $("#speed").text(100 - 15 * parseInt(trial));
        $("#trial").text(trial + 1);
        $("#result").hide();
    ...
    //諦めるボタン
    $("#giveup").hide().unbind().click(function() {
        ending();
    });

    //答える欄
    $("#answer").show().unbind().keydown(function(e){
        if (e.keyCode != 13) {
            return;
        }
        ...
        // 差分チェック
        var res = diff_html(sentence, $(this).val());
        if (!res) return;
        //$(this).val("");
        ...
        $(".mark .aft, .ans .aft").html(" ").hide();

        $(".mark span").each(function(i) {
            if ($(this).hasClass('normal')) {
                $(this).append('<div style="color:red;position:absolute; top:-20px; font-weight:bold; opacity:.7;">O</div>');
                return;
            }
            var miss = '<div style="color:red;position:absolute; top:-20px; font-weight:bold; opacity:.7;">X</div>';
            $(this).append(miss);
            $(".ans span").eq(i).append(miss);
        });
        $(".mark span div").hide();
        $(".ans span div").hide();
        $("#todo").hide();
        ...
        var pt = function(trial, input) {
            var trialpt = 128;
            for (var i = 0; i < trial - 1; i++) trialpt /= 2;
            var inputpt = trialpt;
            for (var i = 0; i < input - 1; i++) inputpt /= 2;
            return trialpt + inputpt;
        }(trial, input);

        $(".mark span.normal, .mark span.aft").each(function(i) {
            var ptc = $("#point span.rate").hide().eq(i).text();
            if ($(this).hasClass("aft") || (0 < ptc)) return;
            $("#point span").eq(i).text(Math.round(pt)).addClass("fix");
        });
    ...
    var ending = function() {
        $("#answer").unbind();
        $("#result, #ending, #stspeed").show();
        $("#speed, #giveup").hide();
        $("#todo").text("Failed!");

        // 得点計算
        var pt = 0;
        var n = $("#point span").size();
        $("#point span.rate").each(function() {
            pt += parseInt($(this).text());
        });
        pt = Math.round(pt / n * 800 / 256);
        if (n == $("#point span.rate.fix").size()) {
            $("#todo").text("CLEAR!");
            pt += 200;
        }
        $("#point").append("<div>point = <span class=emp>" + pt + "</span></div>");
        ...
        $("#listen").unbind().click(function() {
            ...
            ssu.text = sentence;
            ssu.lang = 'en';
            ssu.rate = parseInt($("#stspeed").val()) * 0.01;
            speechSynthesis.speak(ssu);
            ssu.onerror = function() {
                $(this).append("[error]");
            };
...
var load_score = function()
{
    ...
    var m = document.cookie.split(";").shift().match(/score=([^;]+)/);
    if (!m) return {};
    var val = m[1];
    if(!val) return {};
...
var save_score = function(level, qid, pt)
{
    var scores = load_score();
    var key = level.toString(16) + qid.toString(16);
...
var pt2color = function(pt) {
    ...
    for (var i = 0; i < ctable.length; i++) {
        if (pt == ctable[i].pt) return ctable[i].c;
        if (pt < ctable[i].pt) break;
    }
    ...
    var ret = [];
    for (var i = 0; i < 3; i++)
        ret.push(parseInt(t.c[i] - (t.c[i] - b.c[i]) * (t.pt - pt) / (t.pt - b.pt)));
    return ret;
};
...
var menu = function()
{
    ...
    // 問題一覧
    var res = "";
    for (var level in qlist) {
        var slevel = "Difficulty: ";
        slevel += (5 < 1 * level) ? "(undefined)" : ("★".repeat(1 + 1 * level));

        res += "<fieldset class=qlist><legend>" + slevel + "</legend>";
        qlist[level].forEach(function(c, i) {
            res += '<span id="' + level + '_' + i + '">' + (i + 1) + "</span> ";
        });
        res += "</fieldset>";
    }
    $("#qoption").html(res);
    ...
    for(var key in scores) {
        var level = parseInt(key.substr(0,1), 16);
        var qid = parseInt(key.substr(1), 16);
        var pts = function(val) {
            var ret = [];
            for(var s = val; s; s = s.substr(3)){
                ret.push(parseInt(s.substr(0, 3), 16));
            }
            return ret;
        }(scores[key]);

        var pt = pts.reduce(function(prev, current) {
            return prev + current;
        }) / pts.length;

        var $q = $("#qoption fieldset").eq(level).find("span").eq(qid)
            .css("background-color", "rgb(" + pt2color(pt).join(",") + ")")
        $("<div>").html(pts.join(" ")).addClass("history")
            .css("height", (10 * pts.length) + "px").appendTo($q);
        $q.hover(function() {
            $(".history").hide();
            $(this).find(".history").show();
        });
        if (pt == 1000) {
            $q.css({"background-color":""}).addClass("perfect");
        }
    };

    // 成績の凡例色スペクトラム
    if ($("#spectrum div").size() < 100) {
        for(var i = 0; i <= 1000; i+=4) {
            var $line = $('<div>').appendTo("#spectrum");
            $line.css("background-color", 'rgb(' + pt2color(i).join(",") + ')');
            if (i % 100 == 0)
                $line.append("<span>" + i + "</span>");
        }
    }

    //クリックイベント
    $(".qlist span").click(function() {
        var c = $(this).prop("id");
        var id = c.split("_");
        $("#menu").hide();
        $("#main").show();
...
var diff_html = function(a, b)
{
    ...
    var ds = diff(ans, co);
    var bef = aft = "";
    var key = {"=": "normal", "+":"bef", "-":"aft"};

    ds.forEach(function(d) {
        //console.log(d);
        bef += '<span class="' + key[d.edit] + '">' + d.word + '</span> ';
    });
...
var diff = function(arr1, arr2, is_rev) {
    ...
    var snake = function(k) {
        ...
        if (y1 < y2) { // 経路選択
            ...
            var obj = {edit: is_rev ? '+' : '-' , line: x - 1, word: arr1[x - 1]};
        } else {
            var y = y1 + 1;
            var x = y - k;
            var e0 = ed[k - 1 + offset];
            var obj = {edit: is_rev ? '-' : '+', line: y - 1, word: arr2[y - 1]};
        }
        // 選択した経路を保存
...
"Do you know Adam? - Yes I do. I know him well. He's a good friend of mine.",
// 005. アダムを知っていますか。", "はい、彼をよく知っています。ぼくのよい友だちです。
...
"Do you play any musical instruments?", "Well, I Play the violin, but I'm not very good at it.",
// 012. 何か楽器はひくの？", "ええと、バイオリンをひくけど、あまりじょうずじゃないんだ。
...
"I like your T-shirt, Sarah. I actually have one like it, too.",
...
"Be careful, John. Don't touch the pot. It's very hot.",
...
"How many classes do you have today?", "Six. The last one ends at five thirty.",
// 022. 今日はいつく授業あるの？", "6つ。最後の授業は5時半に終わるよ。

"Ms. Taylor, I have some questions about the test.", "Sure, Fred. Go ahead.",
...
'Sayaka, how do you read this word?", "Tegami. It means "letter.',
...
"Did you see my key anywhere?", "Yes. It was in the bedroom. It was under the chair.",
...
"Your eyes are red, James.", "I'm sleepy. I slept for only two hours last night.",
...
"I'll write you again soon. Take care. Sincerely, Pam.",
// 075. またすぐあなたに手紙を書きます。気をつけてね。敬具、パム。

"Don't worry, Ann. Everything will be fine. Trust me.",
...
"Is ABC University near here?", "Yes. Go straight for two blocks and you'll see it on your right.",
...
"Tom's studying in his room. He has an important exam tomorrow.",
...
"Dave, would you go and get some eggs at the supermarket?",
...
// 107. 彼は公園のベンチにｎすわって絵を描くのが好きです。

"Chris, I'd like introduce my friend to you. This is Gordon.",
...
"Yumi, can I have a piece of paper? I forgot to bring my notebook. － Sure. Here you are.",
...
// 139. The purpose of this meeting is to choose our next leader.
...
"I like listening to the radio. There are many interesting language programs.",
...
"Sam, did you enjoy yourself at the party yesterday?", "Yes, I had a very good time. Thanks for inviting me.",
...
"I'm surprised. You know a lot about American culture.", "Yes. I'm very interested in the United States.",
...
"I'll tell you the truth. But it's a secret so don't tell anybody.", "I won't. I promise.",
...
"Lisa lay down on her bed, spread her arms, an breathed deeply.",
...
"Which train should I take, the A Line or B Line?", "Well, it's cheaper if you take the A Line. But the B Line is faster.",
// 197. A線とB線、どちらの電車に乗るべきですか。", "まぁ、A線に乗るとより安いけど、B線のほうが速いよ。
...
"Cindy, please open the door.", "No, go away! I don't want to talk to anyone. Leave me alone.",
...
"Those boxes look heavy. I'll help you carry them.", "Oh, thanks, Greg. That's kind of you.",
// 234. それらの箱は重そうだね。運ぶの手伝ってあげるよ。", "あ、ありがとう、グレッグ。優しいのね。

"Hi, Pat Are you busy next weekend?", "Hold on. Let me check my calender.",
...
"I was shocked when I got the results of the exam. I was expecting a better score.",
...
// 261. アンディ", "はさまざまな国のバナナを比較しました。彼はブラジルで生産されたバナナがいちばん好きでした。
...
"My father is getting fat. He should do more exercise. Also, he should eat less meat and more vegetables.",
